...
public final class ProjectionStores {
     ...
     *         {@code projectionType} is not an interface, or no generated
     *         implementation is available
     * @throws IllegalStateException if the generated implementation is
     *         malformed or cannot be instantiated
     */
    public static <T> ProjectionStore<T> create(
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "No generated projection store was found for "
                            + projectionType.getName()
                            + ". Ensure the interface is annotated with "
                            + "@ProjectionSchema and annotation processing "
                            + "is enabled.",
                    exception);
        }
        ...
        try {
            ...
            return rethrowConstructorFailure(generatedClassName, exception);
    ...
    private static IllegalStateException malformedImplementation(
            String className, Exception cause) {
    ...
    private static <T> ProjectionStore<T> rethrowConstructorFailure(
            String className, InvocationTargetException exception) {
        ...
        throw new IllegalStateException(
                "Generated projection store constructor failed: " + className,
                cause);
    }
}
